class Scoresheet {
  ...
  init(controller, results, multiplayer, touchEnabled) {
    ...
    for (var i in results) {
      this.results[player0][i] =
        results[i] === null ? null : results[i].toString();
    }
    ...
    assets.sounds["bgm_result"].playLoop(3, false, 0, 0.718, 31.062);
  ...
  startRedraw() {
    ...
    if (!this.multiplayer) {
      ...
      var tetsuoHanaBg =
        "url('" +
        assets.image[
          "results_tetsuohana" + (debugObj.state === "closed" ? "" : "2")
        ].src +
        "')";
  ...
  redraw() {
    ...
    if (
      (this.state.screen !== "fadeIn" || elapsed >= 1000) &&
      !this.scoreSaved
    ) {
      this.saveScore();
    }
    ...
    ctx.fillStyle = this.multiplayer
    ...
    var failedOffset = rules.clearReached(this.results[this.player[0]].gauge)
      ? 0
      : -2000;
    if (players === 2 && failedOffset !== 0) {
      ...
      if (
        p2results &&
        this.controller.syncWith.game.rules.clearReached(p2results.gauge)
      ) {
        failedOffset = 0;
      }
    }
    if (elapsed >= 3100 + failedOffset) {
      for (var p = 0; p < players; p++) {
        ...
        ctx.globalAlpha =
          Math.min(1, Math.max(0, (elapsed - (3100 + failedOffset)) / 500)) *
          0.5;
    ...
    if (elapsed >= 0) {
      ...
      this.draw.alpha(Math.min(1, elapsed / 400), ctx, (ctx) => {
        ...
        for (var p = 0; p < players; p++) {
          ...
          ctx.drawImage(
            assets.image["difficulty"],
            0,
            144 * this.difficulty[results.difficulty],
            168,
            143,
            300,
            150,
          ...
          var defaultName =
            p === 0 ? strings.defaultName : strings.default2PName;
          if (p === this.player[0]) {
          ...
          ctx.fillStyle =
            p === 1 ? "rgba(195, 228, 229, 0.8)" : "rgba(255, 224, 216, 0.8)";
          ctx.fill();
          this.draw.roundedRect({
            ctx: ctx,
            x: 556,
          ...
          this.draw.layeredText(
            ...
            [{ fill: "#fff" }, { outline: "#000", letterBorder: 0.5 }]
          );
          ...
          this.draw.layeredText(
            ...
            [{ outline: "#000", letterBorder: 8 }, { fill: grd }]
          );
          this.draw.layeredText(
            {
              ctx: ctx,
            ...
            [{ outline: "#000", letterBorder: 8 }, { fill: "#ffc700" }]
          );
        }
        ctx.restore();
      });
    ...
    if (!this.multiplayer) {
      if (elapsed >= 400 && elapsed < 3100 + failedOffset) {
        ...
        if (
          ...
          this.tetsuoHanaClass !== "failed"
        ) {
          if (this.tetsuoHanaClass) {
            this.tetsuoHana.classList.remove(this.tetsuoHanaClass);
          }
          this.tetsuoHanaClass = this.rules[this.player[0]].clearReached(
            this.results[this.player[0]].gauge
          )
            ? "dance"
            : "failed";
      ...
      for (var p = 0; p < players; p++) {
        ...
        if (crownType !== null) {
          ...
          this.draw.alpha(this.draw.easeIn(amount), ctx, (ctx) => {
            ...
            if (
              this.state.screen === "fadeIn" &&
              elapsed >= 1200 &&
              !this.state["fullcomboPlayed" + p]
            ) {
    ...
    if (elapsed >= 2400 + noCrownResultWait) {
      ...
      if (!this.state["countupTime0"]) {
        ...
        for (var p = 0; p < players; p++) {
          ...
          var currentTime =
            3100 + noCrownResultWait + results.points.length * 30 * this.frame;
        ...
        for (var i in printNumbers) {
          ...
          for (var p = 0; p < players; p++) {
            ...
            var currentTime =
              lastTime +
              500 +
              results[printNumbers[i]].length * 30 * this.frame;
      ...
      for (var p = 0; p < players; p++) {
        ...
        var points = this.getNumber(
          results.points,
          3100 + noCrownResultWait,
          elapsed
        );
        var scale = 1.3;
        ...
        if (!this.state["countupTime" + p]) {
          ...
          var lastTime =
            3100 +
            noCrownResultWait +
            results.points.length * 30 * this.frame +
            1000;
          for (var i in printNumbers) {
            times[printNumbers[i]] = lastTime + 500;
            lastTime =
        ...
        for (var i in printNumbers) {
          ...
          this.draw.layeredText(
            ...
            [{ outline: "#000", letterBorder: 9 }, { fill: "#fff" }]
          );
        }
    ...
    if (
      this.session &&
      !this.state.scoreNext &&
      this.state.screen === "scoresShown" &&
      ms - this.state.screenMS >= 10000
    ) {
  ...
  saveScore() {
    if (this.controller.saveScore) {
      ...
      var clearReached = this.controller.game.rules.clearReached(
        this.resultsObj.gauge
      );
      var crown = "";
      if (clearReached) {
      ...
      if (!oldScore || oldScore.points <= this.resultsObj.points) {
        if (
          oldScore &&
          (oldScore.crown === "gold" || (oldScore.crown === "silver" && !crown))
        ) {
        ...
        scoreStorage
          .add(hash, difficulty, this.resultsObj, true, title)
          .catch(() => {
            this.showWarning = { name: "scoreSaveFailed" };
          });
      } else if (
        oldScore &&
        ((crown === "gold" && oldScore.crown !== "gold") ||
          (crown && !oldScore.crown))
      ) {
